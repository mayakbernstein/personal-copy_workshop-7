...
.qmd is located in the code folder and abalone growth is located in the data folder

Note the file path within the `read_csv()` function. How is this different from how we've stored and read data before?

everything before was in the same folder, and now we have multiple folders so we need to specify the file path
...
*they all tell information about the data but it is different based on each function.*
...
abalone_plot <- ggplot(data = abalone, #which data I am talking about
                       aes(x = temperature, # x axis is temperature
                           y = growth)) + # y axis is growth
  geom_point( #make a scatter plot
    size = 10, #make the size of the plots bigger
             color = "blue")+ #make the points blue
  theme_classic()#classic theme takes away gridlines 

abalone_plot #show plot
```
...
*Slope: insert slope and SE here*\
*y-intercept: insert intercept and SE here*
...
Using your notes, write 2-4 sentences about the model results. Start with a broad overarching result (model significance and correlation), then describe specifics of the model estimates (slope and coefficient if relevant), and model predictions (at a specific predictor value, the model predicts \_\_\_\_\_ response value).

*write about your model here*
